...
class Trainer:
    ...
    def __init__(
        ...
        tracker_init_kwargs: dict | None = None,
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            tracker_init_kwargs: Per-tracker init kwargs passed to ``accelerator.init_trackers``,
                e.g. ``{"wandb": {"project": "my-project", "entity": "my-team"}}``.
            tracker_config: Hyperparameter dict logged as the run config in the tracker
                (e.g. wandb config panel). Auto-populated by ``load()`` from the full config;
                override via ``config.train.tracker_config`` if needed.
        ...
        tracker_project_name = kwargs.pop('tracker_project_name', project_name)
        ...
        self.accelerator.init_trackers(tracker_project_name, config=tracker_config, init_kwargs=tracker_init_kwargs or {})
        if allow_tf32:
    ...
    def _get_deepspeed_config(self) -> dict[str, Any]:
        ...
        if plugin is not None and getattr(plugin, 'deepspeed_config', None) is not None:
            return plugin.deepspeed_config
        return getattr(self.accelerator, 'deepspeed_config', {})

    def _is_deepspeed_zero3(self) -> bool:
        if self.distributed_type != DistributedType.DEEPSPEED:
            return False
        zero_config = self._get_deepspeed_config().get('zero_optimization', {})
        return zero_config.get('stage', 0) == 3

    def _enable_gradient_checkpointing_recursively(self, module: Module) -> bool:
        if hasattr(module, 'enable_gradient_checkpointing'):
            module.enable_gradient_checkpointing()
            return True
        enabled = False
        for child in module.children():
            enabled = self._enable_gradient_checkpointing_recursively(child) or enabled
        return enabled
    ...
    def load(cls, config_or_path: Any):
        ...
        # tracker_config may be explicitly set in config.train; pop it so it isn't passed twice.
        tracker_config = train_kwargs.pop('tracker_config', None)
        if tracker_config is None:
            # Auto-build from the full config so lr/optimizer/scheduler are captured.
            _HPARAM_KEYS = {
                'max_steps', 'max_epochs', 'gradient_accumulation_steps', 'mixed_precision',
                'log_interval', 'checkpoint_interval', 'with_ema', 'activation_checkpointing',
            }
            tracker_config = {k: train_kwargs[k] for k in _HPARAM_KEYS if k in train_kwargs}
            if hasattr(config, 'optimizers'):
                tracker_config['optimizer'] = config.optimizers.to_dict()
            if hasattr(config, 'schedulers'):
                tracker_config['scheduler'] = config.schedulers.to_dict()
        trainer = cls(project_dir=config.project_dir, tracker_config=tracker_config, **train_kwargs)
        trainer.prepare(
    ...
    def remove_checkpoint(self, total_limit: int | None = None) -> None:
        ...
        if not self.checkpoint_save_optimizer:
            # Strip optimizer from every currently retained checkpoint. The new
            # checkpoint about to be saved writes its own optimizer after this,
            # so post-save only the freshly-saved one carries optimizer state.
            # (Previous `checkpoints[:-1]` kept the prior-save's optimizer, which
            # under total_limit=2 meant both retained checkpoints always had one.)
            for checkpoint in checkpoints:
                if self.distributed_type == DistributedType.DEEPSPEED:
    ...
    def get_dataloaders(self, data_config: Any) -> DataLoader:
        ...
        persistent_workers = bool(data_config.get('persistent_workers', False)) and num_workers > 0
    ...
    def apply_activation_checkpointing(self, models: list[Module] | None = None) -> None:
        if self.activation_checkpointing and self.activation_class_names is not None:
            ...
            for model in models:
                # DeepSpeed ZeRO-3 can conflict with PyTorch's CheckpointWrapper-based
                # activation checkpointing because ZeRO attaches bookkeeping fields to
                # wrapped modules. Prefer model-native gradient checkpointing when
                # available to avoid wrapper/ZeRO hook incompatibilities.
                if self._is_deepspeed_zero3():
                    unwrapped_model = self.accelerator.unwrap_model(model, keep_torch_compile=False)
                    self._enable_gradient_checkpointing_recursively(unwrapped_model)
                    continue
    ...
    def _get_optimizer_param_groups(self) -> list[dict[str, Any]]:
        ...
        if hasattr(optimizer, 'optimizer') and hasattr(optimizer.optimizer, 'param_groups'):
            return list(optimizer.optimizer.param_groups)
        return []

    @staticmethod
    def _lr_to_float(value: Any) -> float:
        if isinstance(value, torch.Tensor):
            return float(value.detach().float().item())
        return float(value)

    def _get_logged_lrs(self) -> dict[str, float]:
        param_groups = self._get_optimizer_param_groups()
        if not param_groups:
            return {'lr': float(self.scheduler.get_last_lr()[0])}
        ...
        for i, group in enumerate(param_groups):
            if 'lr' not in group:
                continue
            lr_value = self._lr_to_float(group['lr'])
            ordered_lrs.append(lr_value)
            group_name = str(group.get('name', f'group_{i}'))
            named_lrs[group_name] = lr_value

        outputs = {}
        if 'base' in named_lrs:
            outputs['lr'] = named_lrs['base']
        elif ordered_lrs:
            outputs['lr'] = ordered_lrs[0]
        else:
            outputs['lr'] = float(self.scheduler.get_last_lr()[0])

        if 'action' in named_lrs:
            outputs['lr_action'] = named_lrs['action']
        return outputs
    ...
    def print_step(self) -> None:
        ...
        if self.cur_step % self.log_interval == 0:
            ...
            avg_batch_time = time_cost / self.log_interval
            ...
            lr_outputs = self._get_logged_lrs()
            outputs.update(lr_outputs)
            lr = outputs['lr']
            outputs['avg_batch_time'] = avg_batch_time
            outputs['samples_per_sec'] = speed
            self._outputs.clear()
            ...
            if 'lr_action' in outputs:
                msg += ', lr_action: %.3e' % outputs['lr_action']
